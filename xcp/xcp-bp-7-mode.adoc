...
summary: 이 섹션에서는 7-Mode에서 작동하는 NetApp Data ONTAP 에서 ONTAP 으로 데이터를 마이그레이션하는 자세한 단계를 제공합니다. 
---
...
2 entries were displayed.
....
. 대상 시스템에 최소한 하나의 비루트 집계가 있는지 확인합니다.  집계는 정상입니다.
+
....
...
. 대상 클러스터 시스템에 스토리지 가상 머신(SVM)을 생성합니다.
+
....
...
....
. 대상 SVM에서 FCP, iSCSI, NDMP 및 CIDS 프로토콜을 제거합니다.
+
....
...
dest    nfs
....
. 대상 SVM에 새로운 읽기-쓰기 데이터 볼륨을 만듭니다.  보안 스타일, 언어 설정 및 용량 요구 사항이 소스 볼륨과 일치하는지 확인하세요.
+
....
...
....
. NFS 클라이언트 요청을 처리하기 위해 데이터 LIF를 생성합니다.
+
....
...
....
. SVM 네임스페이스에 대상 NFS 데이터 볼륨을 마운트합니다.
+
....
...
....
. 기본 NFS 내보내기 정책이 대상 SVM에 적용되었는지 확인합니다.
+
....
...
dest             default
....
. 필요한 경우 대상 SVM에 대한 새 사용자 지정 내보내기 정책을 만듭니다.
+
....
...
2 entries were displayed.
....
. NFS 클라이언트에 대한 액세스를 허용하도록 내보내기 정책 규칙을 수정합니다.
+
....
...
2 entries were displayed.
....
. Linux NFS 서버에 연결합니다.  NFS로 내보낸 볼륨에 대한 마운트 지점을 만듭니다.
+
....
[root@localhost /]# cd /mnt
[root@localhost mnt]# mkdir dest
....
. 이 마운트 지점에 대상 NFSv3 내보낸 볼륨을 마운트합니다.
+

NOTE: NFSv3 볼륨은 내보내야 하지만 반드시 NFS 서버에 의해 마운트될 필요는 없습니다.  이러한 볼륨을 마운트할 수 있는 경우 XCP Linux 호스트 클라이언트가 해당 볼륨을 마운트합니다.
...
....
. NFS로 내보낸 마운트 지점에 테스트 파일을 만들어 읽기-쓰기 액세스를 활성화합니다.
+
....
...
NOTE: 읽기-쓰기 테스트가 완료되면 대상 NFS 마운트 지점에서 파일을 삭제합니다.

. XCP가 설치된 Linux 클라이언트 시스템에 연결합니다.  XCP 설치 경로를 찾습니다.
+
....
[root@localhost ~]# cd /linux/
[root@localhost linux]#
....
. 다음을 실행하여 소스 7-Mode NFSv3 내보내기를 쿼리합니다. `xcp show` XCP Linux 클라이언트 호스트 시스템의 명령입니다.
+
....
...
....
. 소스 NFSv3에서 내보낸 경로를 스캔하고 해당 파일 구조의 통계를 인쇄합니다.
+
...
....
. 소스 7-모드 NFSv3 내보내기를 대상 ONTAP 시스템의 NFSv3 내보내기로 복사합니다.
+
....
...
....
. 복사가 완료된 후 소스 및 대상 NFSv3 내보내기에 동일한 데이터가 있는지 확인합니다. 실행하다  `xcp verify` 명령.
+
....
...
. 컷오버 전과 컷오버 중에 실행 `verify` 다시.  소스에 새 데이터나 업데이트된 데이터가 있는 경우 증분 업데이트를 수행합니다. 실행하다  `xcp sync` 명령.
+
....
...
....
. 이전에 중단된 복사 작업을 재개하려면 다음을 실행하세요. `xcp resume` 명령.
+
....
...
. NFSv3 클라이언트 호스트는 7-Mode 스토리지에서 프로비저닝된 소스 NFSv3 내보내기를 마운트 해제하고 ONTAP 에서 대상 NFSv3 내보내기를 마운트해야 합니다.  컷오버에는 정전이 필요합니다.
...
NOTE: NetApp 소스 7-Mode 볼륨이 클라이언트 시스템으로 내보내지고 마운트되었으며 XCP가 이미 Linux 시스템에 설치되어 있다고 가정합니다.  스냅샷 복사본은 마지막 스냅샷 복사본 이후의 증분 변경 사항을 기록하는 볼륨의 특정 시점 이미지입니다.  사용하세요 `-snap` 7모드 시스템을 소스로 사용하는 옵션입니다.
   ...
   Vserver Operational State Stopped Reason: -
            ...
            Is Vserver with Infinite Volume: false
                           QoS Policy Group: -
                               ...
                               IPspace Name: Default
....
. 대상 SVM에서 FCP, iSCSI, NDMP 및 CIFS 프로토콜을 제거합니다.
+
....
CLUSTER::> vserver remove-protocols -vserver dest -protocols fcp,iscsi,ndmp,cifs
...
2 entries were displayed.
....
. 대상 시스템의 NFS 클라이언트에 대한 액세스를 허용하도록 내보내기 정책 규칙을 수정합니다.
+
....
CLUSTER::> export-policy rule modify -vserver dest -ruleindex 1 -policyname xcpexportpolicy -clientmatch 0.0.0.0/0 -rorule any -rwrule any -anon 0
Verify the policy rules have modified
...
User ID To Which Anonymous Users Are Mapped: 65534
               ...
               Honor SetUID Bits in SETATTR: true
                  Allow Creation of Devices: true
....
. 클라이언트가 대상 볼륨에 액세스할 수 있는지 확인합니다.
+
....
CLUSTER::> export-policy check-access -vserver dest -volume dest_nfs -client-ip 10.61.82.215 -authentication-method none -protocol nfs3 -access-type read-write
                                         Policy    Policy       Rule
...
(show scan and sync)
....
. 소스 7-Mode NFSv3 스냅샷(기본)을 대상 ONTAP 시스템의 NFSv3 내보내기로 복사합니다.
+
....
...
NOTE: 이후의 동기화 작업을 위해 이 기본 스냅샷을 보관하세요.
...
. 컷오버 전과 컷오버 중에 실행 `verify` 다시.  소스에 새 데이터나 업데이트된 데이터가 있는 경우 증분 업데이트를 수행합니다.  증분 변경 사항이 있는 경우 이러한 변경 사항에 대한 새 스냅샷 복사본을 만들고 해당 스냅샷 경로를 다음과 함께 전달합니다. `-snap` 동기화 작업을 위한 옵션입니다.
+
...
....
. NFSv3 클라이언트 호스트는 7-Mode 스토리지에서 프로비저닝된 소스 NFSv3 내보내기를 마운트 해제하고 ONTAP 에서 대상 NFSv3 내보내기를 마운트해야 합니다.  이 전환에는 중단이 필요합니다.
...
NOTE: NetApp 소스 NFSv4 볼륨이 클라이언트 시스템으로 내보내지고 마운트되었으며 XCP가 이미 Linux 시스템에 설치되어 있다고 가정합니다.  소스는 ACL을 지원하는 NetApp 7-Mode 시스템이어야 합니다.  ACL 마이그레이션은 NetApp NetApp 만 지원됩니다.  이름에 특수 문자가 포함된 파일을 복사하려면 소스와 대상이 UTF-8 인코딩 언어를 지원하는지 확인하세요.
...
Vserver: dest
 ...
 Default Windows Group:  -
....
. 기본 NFS 내보내기 정책이 대상 SVM에 적용되었는지 확인하세요.
+
....
CLUSTER::> vserver export-policy show -vserver dest
Vserver          Policy Name
...
[root@localhost mnt]# mkdir dest
....
. 이 마운트 지점에 대상 NFSv4 내보낸 볼륨을 마운트합니다.
+

NOTE: NFSv4 볼륨은 내보내야 하지만 반드시 NFS 서버에 의해 마운트될 필요는 없습니다.  이러한 볼륨을 마운트할 수 있는 경우 XCP Linux 호스트 클라이언트가 해당 볼륨을 마운트합니다.
...
[root@localhost linux]#
....
. 다음을 실행하여 소스 NFSv4 내보내기를 쿼리합니다. `xcp show` XCP Linux 클라이언트 호스트 시스템의 명령입니다.
+
....
...
....
. 소스 NFSv4에서 내보낸 경로를 스캔하고 해당 파일 구조의 통계를 인쇄합니다.
+
...
....
. 소스 NFSv4 내보내기를 대상 ONTAP 시스템의 NFSv4 내보내기로 복사합니다.
+
....
...
....
. 후에 `copy` 완료되면 소스 및 대상 NFSv4 내보내기에 동일한 데이터가 있는지 확인하세요. 실행하다  `xcp verify` 명령.
+
....
...
NOTE: 이 작업에는 이전 복사본 인덱스 이름 또는 번호가 필요합니다.

. 이전에 중단된 작업을 다시 시작하려면 `copy` 작업을 실행하세요 `xcp resume` 명령.
+
....
...
NOTE: NetApp 7-Mode와 ONTAP 시스템이 SMB 라이센스를 받았다고 가정합니다.  대상 SVM이 생성되고, 소스 및 대상 SMB 공유가 내보내지고, XCP가 설치되고 라이선스가 부여됩니다.

. SMB 공유에서 파일과 디렉토리를 스캔합니다.
+
....
...
....
. 소스에서 대상 SMB 공유로 파일을 복사합니다(ACL 포함 여부와 관계없이).  다음 예에서는 ACL을 사용한 복사를 보여줍니다.
+
....
...
NOTE: 데이터 집계가 없는 경우 저장소를 사용하여 새 데이터를 생성합니다. `aggr create` 명령.
