...
summary: 이 섹션에서는 보안 정보가 포함된 CIFS 데이터를 소스에서 대상 ONTAP 시스템으로 마이그레이션하는 단계별 절차를 설명합니다. 
---
= ACL을 사용하여 소스 스토리지 박스에서 ONTAP 으로 CIFS 데이터 마이그레이션
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
이 섹션에서는 보안 정보가 포함된 CIFS 데이터를 소스에서 대상 ONTAP 시스템으로 마이그레이션하는 단계별 절차를 설명합니다.
...
NOTE: 데이터 집계가 없는 경우 다음을 사용하여 새 데이터를 만듭니다. `storage aggr create` 명령.
...
....
. SMB 클라이언트 요청을 처리하기 위해 데이터 LIF를 생성합니다.
+
....
...
....
. 기본 내보내기 정책이 대상 SVM에 적용되었는지 확인합니다.
+
....
...
....
. CIF 클라이언트에 대한 액세스를 허용하도록 내보내기 정책 규칙을 수정합니다.
+
....
...
2 entries were displayed.
....
. XCP가 설치된 Windows 클라이언트 시스템에 연결합니다.  XCP 설치 경로를 찾습니다.
+
....
C:\WRSHDNT>dir c:\netapp\xcp
dir c:\netapp\xcp
...
....
. 다음을 실행하여 소스 노드 SMB 내보내기를 쿼리합니다. `xcp show` XCP Windows 클라이언트 호스트 시스템의 명령입니다.
+
....
...
....
. 대상 ONTAP 시스템에서 값으로 제공해야 하는 로컬 사용자 및 로컬 그룹 이름 목록을 가져옵니다. `fallback-user` 그리고 `fallback-group` 인수 경로.
+
....
...
5 entries were displayed
....
. ACL을 사용하여 소스에서 타겟으로 CIF 데이터를 마이그레이션하려면 다음을 실행합니다. `xcp copy` 명령으로 `-acl` 그리고 `–fallback-user/group` 옵션.
+
를 위해 `fallback-user/group` 옵션을 사용하여 Active Directory에서 찾을 수 있는 사용자나 그룹 또는 대상 시스템의 로컬 사용자/그룹을 지정합니다.
...
C:\WRSHDNT>
....
. 만약에 `xcp copy` 오류 메시지가 발생합니다 `ERROR failed to obtain fallback security principal` , 호스트 파일에 대상 상자를 추가합니다.(`C:\Windows\System32\drivers\etc\hosts` ).
+
NetApp 스토리지 대상 상자 항목에는 다음 형식을 사용하세요.
...
....
. 여전히 오류 메시지가 나타나면 `ERROR failed to obtain fallback security principal` 호스트 파일에 대상 상자 항목을 추가한 후에는 대상 시스템에 사용자/그룹이 존재하지 않습니다.
+
....
...
C:\WRSHDNT>
....
. 사용 `xcp copy` ACL(루트 폴더 유무에 관계없이)을 사용하여 CIF 데이터를 마이그레이션합니다.
+
루트 폴더 없이 다음 명령을 실행하세요.
